...
import { x as isString, _ as _export_sfc, B as withInstall } from "./index.f575d9f0.js";
import { A as cloneDeep, d as defineComponent, Q as ref, a6 as onMounted, w as watch, b as unref, o as openBlock, R as createElementBlock, c as createBlock, ap as resolveDynamicComponent } from "./vendor.faf2de98.js";
...
var toCanvas$1 = renderCanvas.bind(null, CanvasRenderer.render);
...
const renderQrCode = ({
  ...
  return getOriginWidth(content, options).then((_width) => {
    ...
    return toCanvas$1(canvas2, content, options);
  });
};
function getOriginWidth(content, options) {
  const _canvas = document.createElement("canvas");
  return toCanvas$1(_canvas, content, options).then(() => _canvas.width);
}
...
const drawLogo = ({ canvas: canvas2, logo }) => {
  ...
  const drawLogoWithCanvas = (image2) => {
    ...
    if (!imageCanvas || !ctx)
      return;
    imageCanvas.drawImage(image2, logoXY, logoXY, logoWidth, logoWidth);
  ...
  setup(props, { emit }) {
    ...
    async function createQrcode() {
      try {
        ...
        if (tag === "img") {
          const url = await toDataURL(renderValue, __spreadValues({
...
const QrCode = withInstall(qrCode);
export { QrCode as Q };
